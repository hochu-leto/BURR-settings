...
'''
Небольшая инструкция по работе с БУРР-30 в тестовом режиме
Для работы блока БУРР-30 в режиме тестового управления по CAN SDO-сообщениям необходимо выполнить следующие действия:
1. Перевести БУРР-30 в тестовый режим: в параметр с адресом 506 DEC записываем значение «0»;
2. Рекомендуется в параметре 403 DEC указать в качестве предельного ограничения в формате «ХХ.ХХ» в амперах минимальное значения тока, достаточного для движения рейки;
3. Для включения электродвигателя рулевой рейки, в параметр по адресу 500 DEC записываем «1»;
4. Для выключения электродвигателя рулевой рейки в параметр по адресу 500 DEC записываем значение «0»;
5. При необходимости выполнить сброс аварии необходимо в параметре по адресу 500 DEC записать значение «5» ;

!!! ОБРАТИТЬ ВНИМАНИЕ НА СЛЕДУЮЩЕЕ !!!
- вне зависимости от состояния дискретного входа «IGN» и от получаемых блоком БУРР-30 команд в приходящих PDO-сообщениях ID=0x314, в тестовом режиме при записи «1» по адресу 500 DEC блок БУРР-30 СРАЗУ ЗАПУСТИТСЯ В РАБОТУ!
- функция проверки соблюдения полярности при подключении электродвигателя при пуске в тестовом режиме не работает, и если полярность подключения электромотора окажется неправильной, рейка на большой скорости рванёт в крайнее положение с максимальным предельным током, указанным в параметре по адресу 403 DEC!
- дополнительные защиты от перегрузки по току в тестовом режиме не работают!
- длительная работа электромотора с большими токами может привести к перегреву его обмоток и поломке!
- в тестовом режиме необходимо следить за токами потребления электромотора рейки!
