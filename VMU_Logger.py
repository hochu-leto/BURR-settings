...
какя-то непонятная фигня, есть подозрения, что это особенность работы марафона - без перзагрузки рн периодически
отваливается . Выход - переход на квайзер и кан-хакер
- нет индикации работы когда считывает параметры
- выбрасывает ошибку при установке задней или передней оси(но ось задаёт или нет)
- в списке осей нет индикации заводской настройки
- не записывает значения параметров из файла настроек в устройство
- не записывает порядок байт - после сохранения
параметров БУРР их нет в листе param_list
- не может переключить рейку с задней на переднюю
